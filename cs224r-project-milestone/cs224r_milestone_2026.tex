...
\subsection{Training and Evaluation Metrics}
We initialize RLOO from the SFT checkpoint and train for 100 update steps with a
constant learning rate of $1\times10^{-5}$, batch size 128, KL coefficient
$0.001$ to the frozen SFT reference, and no entropy bonus. For each prompt we
...
We targeted a group size of 8 but hit memory limits on the H100 and reduced it to
4, which makes the leave-one-out baseline noisier. Mean reward stayed around
$0.25$ to $0.30$ (Figure~\ref{fig:rloo_reward}) and rollout accuracy around
$0.18$ to $0.26$ across the run, stable but without a clear upward trend. The importance
weight is near 1 for most sequences but spikes sharply late in training
(Figure~\ref{fig:rloo_importance}); with gradient clipping disabled that update
was large, but reward recovered right after.
...
\begin{figure}[H]
  ...
  \begin{minipage}{0.48\textwidth}
    ...
    \caption{RLOO loss on the train set. The single spike late in training dominates the vertical scale; loss is otherwise small and stable.}
    \label{fig:rloo_loss}
  \end{minipage}
  \hfill
  \begin{minipage}{0.48\textwidth}
...
\begin{figure}[H]
  ...
  \caption{Mean reward on the train set, which stays in the $0.25$ to $0.30$ range without a clear upward trend over the 100 steps.}
  \label{fig:rloo_reward}
\end{figure}

\subsubsection{Functional Correctness (Pass@$k$)}
Using the same 50 held-out problems and 16 completions per problem, RLOO reaches
pass@1 $24.6\%$ and pass@16 $70.0\%$, below both SFT ($33.2\%$, $74.0\%$) and IPO
($35.9\%$, $74.0\%$). This is consistent with the training curves: over only 100
steps with a group size of 4 and a reward signal that never trended upward, the
policy did not improve over its SFT starting point and slightly regressed. The
importance-weight instability late in training likely contributed by letting a few
mis-weighted updates dominate. Figure~\ref{fig:rloo_pass_k} shows all three
methods on one axis.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{../passk_comparison.png}
  \caption{Pass@$k$ on the held-out set, SFT versus RLOO (IPO also shown for reference). RLOO sits below both baselines at every $k$.}
